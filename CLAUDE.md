...
- **`--memory`/`--cpus` cannot be enforced on Apple container machines.** The guest's `/sys/fs/cgroup` is a namespaced view of a cgroup that Apple's vminitd owns from outside: migrating processes out of the namespace root and delegating domain controllers (`+memory`/`+io`) both return `EOPNOTSUPP`, so runc can never apply memory/cpu limits ("cannot enter cgroupv2 /sys/fs/cgroup/docker with domain controllers"). Only threaded-capable controllers (cpuset/cpu/pids) delegate — `--pids-limit` works. berth detects this and drops `--memory`/`--cpus` with a warning; explicit flags error out.
