# safe-ag-app — macOS Desktop App for safe-agentic (Design)
...
safe-agentic engine. Not a competitor product — a personal tool.
...
- Host git-worktree management UI (safe-ag manages worktrees; engine-side)
...
### safe-agentic advantages surfaced in the app (Supacode has none of these)
...
- Scope: safe-agentic containers only (no host sessions, no other backends).
- Four pillars: embedded terminals, fleet/pipeline dashboard, diff/review UI,
  menubar + native notifications.
- Must not duplicate engine logic: safe-ag CLI remains the single source of truth
  for mutations.
...
Location: `app/` directory in the safe-agentic repo, mirroring `tui/`:
...
│  ├─ CLIRunner     shell safe-ag (mutations)    │  └─ Command palette (⌘K)           │
...
└─────────────────────────────────────────────────────────────────────────────────────┘
    ...
    ~/.safe-ag/state/*.jsonl     safe-ag CLI (JSON out)     PTY → tmux in container
...
- **Hybrid data access.** Reads: import `pkg/{audit,events,fleet,cost,labels,config}`
  directly + Docker labels via `pkg/vmexec`. Writes: shell out to `safe-ag`
  (JSON output where available). No engine logic duplicated in the app.
- **No dependency on `safe-ag server`** in v1. It is read-only and unused by the
...
|---|---|---|
| Poller | 2s snapshot: `docker ps -a` (safe-agentic labels), `docker stats`, CPU probe → Working/Idle; diff vs last snapshot → events | port of `tui/poller.go` |
| EventWatcher | fsnotify on `~/.safe-ag/state/events.jsonl`, `state/pipelines/`; classify via `pkg/events` classifier; dispatch notifications + UI events | `pkg/events` |
| TermManager | one goroutine per open terminal: `creack/pty` running `container machine run safe-agentic docker exec -it <c> tmux attach`; output → per-terminal Wails event stream (base64, ~16ms coalescing); input/resize ← bound methods; close = detach | tmux in container |
| CLIRunner | exec `safe-ag <cmd>`; parse JSON (`list --json`, `output --json`) or exit code; every invocation logged with full argv | safe-ag CLI |
...
- **Agent workspace** (tabs per agent):
  ...
  2. *Diff* — diff2html rendering of `safe-ag diff`; file tree; stage/revert via
...
- **Mutations:** UI → bound Go method → `safe-ag` exec → parse → optimistic store
...
- VM unreachable → full-width banner + one-click `safe-ag vm start` streamed into a
...
- **fsnotify on `~/.safe-ag/state`** — JSONL appends may coalesce; mtime fallback
  covers it.
...
- Extend `safe-ag server` (write methods + SSE) and migrate app + TUI onto it.
